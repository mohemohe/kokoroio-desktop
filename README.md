...
Markdownの描画に[Textual](https://github.com/gonzalezreal/textual)を使用します。初回ビルド時はSwift Package Managerが依存ライブラリをダウンロードします。API モデル・HTTP 通信・Action Cable 接続をローカル Swift Package の `KokoroCore` に分離しています。
...
- TextualによるMarkdown表示（見出し・リスト・引用・コードブロック・表・リンク・文字選択）
...
Markdown内の画像は本文ではリンクとして表示し、画像プレビューは以下のメディア表示に集約します。センシティブなメディアは表示ボタンを押すまで読み込みません。
